...
initialCI_list<-c(5.360, 6.037, 6.472, 6.021, 4.267, 6.924, 5.585, 5.862, 8.023, 6.955, 4.720, 5.820, 6.929, 6.101, 6.953, 7.536, 6.938, 6.538, 6.370, 7.339)
initialCI<-data.frame(initialCI_list)
ggplot(initialCI,aes(x=initialCI_list))+geom_histogram(bins = 8, fill="#69b3a2", color="#e9ecef")+theme_classic()+xlab("Condition index")+ylab("Frequency")
summary(initialCI_list)

shapiro.test(initialCI_list)