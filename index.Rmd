...
- [First Sampling Day - Graphical Results](#graphical-results)
- [Initial Condition Index](#initial-condition-index)
- [Turbidity](#turbidity)
...
```{r include=FALSE, initialHeights-analysis}
...
```{r include=FALSE, initialHeights-analysis2}
...
```{r warning=FALSE, echo=FALSE, initialShellHeightDist, fig.width=4, fig.height=3, fig.align='center'}
library(ggplot2)
...
SamplingOne<-read.csv("6_14_22_including_lost_bag.csv")
...
bag1length.normality<-shapiro.test(bag1length) #Normal
bag2length<- FBout[FBout$Bag == "2","Length"]
bag2length.normality<-shapiro.test(bag2length) #Normal
bag3length<- FBout[FBout$Bag == "3","Length"]
bag3length.normality<-shapiro.test(bag3length) #Normal
bag4length<- FBout[FBout$Bag == "Rep","Length"]
bag4length.normality<-shapiro.test(bag4length) #Normal
```
<!--Testing width for homoscedasticity (no violation)-->
```{r include=FALSE, message=FALSE, warning=FALSE }
width.levene<-leveneTest(Width~Bag, data=FBout)
```
<!--Testing width for normality (assumption violated)-->
```{r include=FALSE}
bag1width<- FBout[FBout$Bag == "1","Width"]
bag1width.normality<-shapiro.test(bag1width) #Normal
bag2width<- FBout[FBout$Bag == "2","Width"]
bag2width.normality<-shapiro.test(bag2width) #Not normal, p=0.0231
bag3width<- FBout[FBout$Bag == "3","Width"]
bag3width.normality<-shapiro.test(bag3width) #Normal
bag4width<- FBout[FBout$Bag == "Rep","Width"]
bag4width.normality<-shapiro.test(bag4width) #Not normal, p=0.04589
```
...
No assumptions were violated for height and length, so a one-way ANOVA was used to determine if significant differences existed for these metrics between any of the four outside ADPI bags— the three original bags plus the replacement bag. The shell width distributions were non-normal for both the original bag #2 and the replacement bag #2, and transformation was unsuccessful. As such, the nonparametric Kruskal-Wallis test was used in place of the ANOVA.

Height: no significant differences observed (One-way ANOVA, F(3,24)=1.427, p=0.238) <br>
Length: no significant differences observed (One-way ANOVA, F(3,24)=2.547, p=0.059) <br>
Width: no significant differences observed (Kruskal-Wallis test, H(3)=2.232, p=0.526) <br>

**Conclusion:** no significant differences were found between shell height, length, or width in any of the outside ADPI bags, including the replacement bag. For future analysis of growth and shell morphology, the replacement bag will be analyzed as if it were the original outside bag #2. However, biofouling did appear to be more severe on the replacement oysters than on the oysters in the original outside ADPI bags. As such, when oysters are selected from each treatment for biofouling analysis at the conclusion of the experiment, the outside FB sample will be collected from only bags #1 and #3.

```{r echo=FALSE, figures-side, fig.show="hold", out.width="33%"}
ggplot(data = FBout, aes(x = Bag, y = Height))+geom_boxplot()+theme_classic()+xlab("Bag")+ylab("Height (mm)")+theme(text = element_text(size=15))+scale_y_continuous(limits=c(0,75))
ggplot(data = FBout, aes(x = Bag, y = Length))+geom_boxplot()+theme_classic()+xlab("Bag")+ylab("Length (mm)")+theme(text = element_text(size=15))+scale_y_continuous(limits=c(0,50))
ggplot(data = FBout, aes(x = Bag, y = Width))+geom_boxplot()+theme_classic()+xlab("Bag")+ylab("Width (mm)")+theme(text = element_text(size=15))+scale_y_continuous(limits=c(0,25))
```
Figure #2: Shell height, length, and width of oysters in outside floating bags
<br>
...
```{r echo=FALSE, fig.width=4, fig.height=3, fig.align='center'}
SamplingNew<-read.csv("6_14_22.csv")
...
```
Figure #8: Oyster shell cup ratio (width/height) across gear types and locations (n=32 oysters per gear/location combination).


```{r echo=FALSE, fig.width=4, fig.height=3, fig.align='center'}
ggplot(data = SamplingNew, aes(x = Gear, y = Shell.shape, fill=Location))+geom_boxplot()+scale_y_continuous(limits=c(0,8))
```
Figure #9: Oyster shell shape index across gear types and locations (n=32 oysters per gear/location combination).
...
meanInitialCI<-mean(initialCI_list)
SDInitialCI<-sd(initialCI_list)
...
```
Figure #8: Initial condition index distribution (n=20)

A random sample of 20 oysters from the population used to stock all experimental gear was collected on 6/16 and assessed for initial condition index following the procedure detailed by Walton et al., 2013, which calculates condition index using the formula from Abbe and Albright (2003): CI = (dry tissue weight)/(whole wet weight - dry shell weight) *100. 


Condition index was normally distributed (Shapiro-Wilk test; W=0.973; p=0.822) with a mean of `r round(meanInitialCI, digits=2)` and a standard deviation of `r round(SDInitialCI, digits=2)`.
...
df2<-data_summary(turbidity, "Turbidity", 
...
#Plot
ggplot(df2, aes(x=Date, y=mean, group=Location, color=Location)) + 
  geom_line() +
  geom_point()+
  geom_errorbar(aes(ymin=mean-SE, ymax=mean+SE), width=.2,
                position=position_dodge(0.05))+theme_classic()+scale_y_continuous(limits=c(0,4))+ylab("Turbidity (NTU)")
```
Figure #9: Average turbidity inside vs. outside the pound (mean±SEM).