...
rownames(randTable)<-c("FB_in", "FB_out", "OG_in", "OG_out")
...
Similar code can be used to generate a table to randomly select the bags to be pulled from within a given 4-pack OysterGro cage.

```{r echo=FALSE}
randTableOG<-data.frame()
for (i in 1:3) {
  newRow<-sample(1:4, 30, replace=TRUE)
  randTableOG<-rbind(randTableOG,newRow)
}
colnames(randTableOG)<-c(1:30)
rownames(randTableOG)<-c("Cage 1", "Cage 2", "Cage 3")
kable(randTableOG)
```
