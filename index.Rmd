...
rownames(randTable)<-c("Bags_in", "Bags_out", "Cages_in", "Cages_out")
...
kable(randTable)
```
This code segment can be re-run each day that the shell dimensions will be sampled to generate a new table of random numbers. When sampling for biofouling or condition index, the "j" loop can be altered to run 15 times instead of 30.

Similar code can be used to generate a table to randomly select the bags to be pulled from within a given 4-pack floating cage.

```{r echo=FALSE}
randTableFC<-data.frame()
for (i in 1:3) {
  newRow<-sample(1:4, 30, replace=TRUE)
  randTableFC<-rbind(randTableFC,newRow)
}
colnames(randTableFC)<-c(1:30)
rownames(randTableFC)<-c("Cage 1", "Cage 2", "Cage 3")
kable(randTableFC)
```
